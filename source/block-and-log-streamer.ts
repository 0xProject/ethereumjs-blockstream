...
export class BlockAndLogStreamer<TBlock extends Block, TLog extends Log> {
	...
	public readonly reconcileNewBlock = async (block: TBlock): Promise<void> => {
		try {
			...
			// NOTE: this catch block may be hit multiple times for a single failure root cause, thus we need to be careful to only do idempotent operations in here something went wrong, rollback to last checkpoint
